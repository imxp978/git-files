<section id="member">
    ...
    <div class="container text-center">
        <?php if (isset($_SESSION['login']) && $_SESSION['login']) { ?>
            ...
            <hr>
            <?php
            $SQLstring_order = sprintf("SELECT uorder.orderid, uorder.cname, uorder.phone, uorder.address, uorder.create_time as ordertime, uorder.remark, ms1.msname as payment_method, ms2.msname as status
                FROM multiselect as ms1, multiselect as ms2, uorder
                WHERE ms2.msid=uorder.status AND ms1.msid=uorder.payment_method AND uorder.user_id=%d
                ORDER BY uorder.create_time DESC", $_SESSION['id']);
            $orders = $link->query($SQLstring_order);
            if ($result = $orders->rowCount()) {
            ?>
                <div class="container">
                    <div class="accordion accordion-flush" id="accordionFlushExample">
                        <div class="container">
                            <div class="row text-start d-none d-lg-flex d-xl-flex d-xxl-flex">
                                    ...
                                    <div class="accordion-body row text-center">
                                        <div class="container col-10 border-bottom border-end bg-gray">
                                            ...
                                            while ($data2 = $items->fetch()) { ?>
                                                <div class="col-12 row text-md-end text-lg-start d-flex align-items-center">
                                                    <div class="col-2 "><img src="images/product_images/<?php echo $data2['img_file']; ?>" width="50px" alt=""></div>
                                                    <div class="col-4 "><?php echo $data2['p_name']; ?></div>
                                                    <div class="col-2 text-end">$: <?php echo $data2['p_price']; ?></div>
                                                    <div class="col-1 text-end"><?php echo $data2['qty']; ?></div>
                                                    <div class="col-2 text-end">$: <?php echo $data2['p_price'] * $data2['qty']; ?></div>
